...
async function cfAI(prompt) {
  ...
  const res = await fetch(
    `https://api.cloudflare.com/client/v4/accounts/${CF_ACCOUNT}/ai/run/${CF_MODEL}`,
    {
      ...
      body: JSON.stringify({
        messages: [
          { role: 'system', content: 'You are an email outreach expert. Return markdown only.' },
  ...
  for (const query of searchQueries) {
    try {
      ...
      if (res.ok) {
        ...
        const nameRegex = /<span[^>]*class="[^"]*entity-result__title-text[^"]*"[^>]*>([^<]*)<\/span>/gi;
        let match;
        while ((match = nameRegex.exec(html)) !== null) {
          const name = match[1].trim();
          if (name && name.includes(' ') && !name.includes('LinkedIn')) {
            ...
            contacts.push({
              ...
              emailPatterns: guessEmailPatterns(first.toLowerCase(), last.toLowerCase(), company.toLowerCase().replace(/\s+/g, '') + '.com'),
            });
          }
        }
      }
...
function showFollowups(data) {
  ...
  for (const item of due) {
    console.log(`  → ${item.company} (${item.role}) — ${item.daysSince} days ago`);
    console.log(`    Last email: ${item.subject}`);
    console.log(`    Follow-up: "Hi, just bumping this up. Still very interested in ${item.role} at ${item.company}."`);
    console.log('');
  }
}
...
async function main() {
  ...
  if (!COMPANY) {
    console.error('Usage:');
    console.error('  node scripts/emailOutreach.mjs --company "Stripe" --role "SWE"');
  ...
  if (contacts.length === 0) {
    ...
    console.log(`   3. Try common patterns: cto@${COMPANY.toLowerCase().replace(/\s+/g, '')}.com`);
    return;
  }
  ...
  contacts.forEach((c, i) => {
    ...
    console.log(`     Guessed emails: ${c.emailPatterns.slice(0, 3).join(', ')}`);
  });
  ...
  console.log('   3. Log: node scripts/emailOutreach.mjs --log "Company" "note"');
  console.log('   4. Follow up in 4-5 days: node scripts/emailOutreach.mjs --followup');
}
