...
import { loadActiveProfile, getProfileTargetRoles } from './lib/profile.mjs';
...
function runScan(query, location) {
  return new Promise((resolvePromise, reject) => {
    const child = spawn(process.execPath, [resolve(ROOT, 'scripts/scan.mjs'), query, location], {
      cwd: ROOT,
    });
    let stdout = '';
    let stderr = '';
...
async function main() {
  ...
  const queries = QUERY.toLowerCase() === 'auto' ? getProfileTargetRoles(profile) : [QUERY];
  ...
  const filtered =
    MIN_SCORE > 0 ? evaluated.filter((j) => (j.evaluation.overall || 0) >= MIN_SCORE) : evaluated;
