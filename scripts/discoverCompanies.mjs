...
import { loadActiveProfile, getProfileSkills } from './lib/profile.mjs';
...
const ATS = argVal('ats', 'greenhouse,lever,ashby').split(',').map(a => a.trim());
...
async function discoverGreenhouse() {
  ...
  const boards = [
    ...
    'planetscale', 'railway', 'render', 'fly', 'cloudflare', 'twilio',
    'sendgrid', 'hubspot', 'salesforce', 'dropbox', 'slack', 'discord',
    'spotify', 'netlify', 'heroku', 'digitalocean', 'gitlab', 'github',
    'bitbucket', 'atlassian', 'mongodb', 'elastic', 'snowflake', 'databricks',
    'openai', 'anthropic', 'cohere', 'hugging', 'replicate', 'weights',
    'scale', 'snorkel', 'labelbox', 'Weights & Biases', 'neptune',
  ];
  
  const companies = [];
  
  for (const board of boards) {
    const data = await fetchWithTimeout(`https://boards-api.greenhouse.io/v1/boards/${board}/jobs`);
    if (data?.jobs?.length) {
...
async function discoverLever() {
  ...
  const boards = [
    'airtable', 'balance', 'calendly', 'canva', 'cats', 'clever',
    'codecademy', 'codility', 'duolingo', 'figma', 'gusto', 'hashicorp',
    'lever', 'mixpanel', 'ned', 'postman', 'ramp', 'robinhood', 'shogun',
    'snap', 'spotify', 'upstart', 'western', 'ziprecruiter',
  ];
  
  const companies = [];
  
  for (const board of boards) {
    const data = await fetchWithTimeout(`https://api.lever.co/v0/postings/${board}?mode=json`);
...
async function discoverAshby() {
  ...
  const boards = [
    'postman', 'linear', 'vercel', 'supabase', 'planetscale', 'railway',
    'render', 'fly', 'cloudflare', 'twilio', 'sendgrid', 'hubspot',
    'salesforce', 'dropbox', 'slack', 'discord', 'spotify', 'netlify',
    'heroku', 'digitalocean', 'gitlab', 'github', 'bitbucket', 'atlassian',
  ];
  
  const companies = [];
  
  for (const board of boards) {
    const data = await fetchWithTimeout(`https://api.ashbyhq.com/posting-api/job-board/${board}`);
    if (data?.jobs?.length) {
      const jobs = data.jobs.slice(0, 10).map(j => ({
        title: j.title,
        url: j.jobUrl,
...
async function main() {
  ...
  if (QUERY) {
    filtered = allCompanies.filter(c => {
      const matchTitle = c.jobs.some(j => j.title.toLowerCase().includes(QUERY));
      const matchDept = c.jobs.some(j => j.department.toLowerCase().includes(QUERY));
  ...
  results.forEach((c, i) => {
    console.log(`  ${i + 1}. ${c.name} (${c.ats}) — ${c.jobCount} open roles`);
  ...
  console.log('  boards:');
  results.filter(c => c.ats === 'greenhouse').forEach(c => {
    console.log(`    - {slug: ${c.name.toLowerCase()}, name: "${c.name}"}`);
  });
  console.log('\nlever:');
  console.log('  boards:');
  results.filter(c => c.ats === 'lever').forEach(c => {
    console.log(`    - {slug: ${c.name.toLowerCase()}, name: "${c.name}"}`);
  });
  console.log('\nashby:');
  console.log('  boards:');
  results.filter(c => c.ats === 'ashby').forEach(c => {
    console.log(`    - {slug: ${c.name.toLowerCase()}, name: "${c.name}"}`);
  });
}
