...
const LOCATION =
  args.find((a, i) => !a.startsWith('--') && args[i - 1] !== '--mock' && i > 0) || 'Remote';

const fetchT = (url, opts = {}, ms = FETCH_TIMEOUT_MS) =>
  ...
  if (snippet) {
    ...
    for (const pattern of experiencePatterns) {
      ...
      if (match) {
        ...
        if (years >= MAX_EXPERIENCE_YEARS) return false; // Skip roles requiring 5+ years
      }
    }
  }
...
function matchesLocationFilter(location) {
  ...
  if (searchConfig.allow_remote && (locLower === 'remote' || locLower.includes('remote')))
    return true;
...
async function scanRemoteOK() {
  ...
  try {
    ...
    for (const item of data.slice(1)) {
      ...
      if (!matchesSearch(`${item.position} ${item.company} ${(item.tags || []).join(' ')}`))
        continue;
      ...
      jobs.push({
        ...
        source: 'remoteok',
        tags: item.tags || [],
        snippet: (item.description || '').replace(/<[^>]*>/g, '').substring(0, SNIPPET_MAX_LENGTH),
        posted,
      });
    }
  } catch (e) {
...
async function scanArbeitnow() {
  ...
  try {
    ...
    for (const item of data.data) {
      ...
      if (!matchesSearch(`${item.title} ${item.company_name} ${(item.tags || []).join(' ')}`))
        continue;

      const posted = item.created_at
        ? new Date(typeof item.created_at === 'number' ? item.created_at * 1000 : item.created_at)
            .toISOString()
            .split('T')[0]
        : 'Unknown';
      if (!isFreshEnough(posted)) continue;
...
async function scanFindwork() {
  ...
  try {
    ...
    for (const item of data.results) {
      if (isBlacklisted(item.company_name)) continue;
      if (!matchesSearch(`${item.role} ${item.company_name} ${(item.keywords || []).join(' ')}`))
        continue;
      ...
      jobs.push({
        ...
        source: 'findwork',
        tags: item.keywords || [],
        snippet: (item.text || '').replace(/<[^>]*>/g, '').substring(0, SNIPPET_MAX_LENGTH),
        posted,
      });
    }
  } catch (e) {
...
async function scanFreehire() {
  ...
  try {
    ...
    for (const item of data.jobs) {
      if (isBlacklisted(item.company)) continue;
      if (!matchesSearch(`${item.title} ${item.company} ${(item.skills || []).join(' ')}`))
        continue;
      ...
      jobs.push({
        ...
        snippet: item.description
          ? item.description.replace(/<[^>]*>/g, '').substring(0, SNIPPET_MAX_LENGTH)
          : '',
        posted,
      });
    }
  ...
  try {
    ...
    for (const item of data.jobs) {
      ...
      jobs.push({
        ...
        url:
          item.absolute_url || item.url || `https://boards.greenhouse.io/${slug}/jobs/${item.id}`,
        source: `greenhouse:${slug}`,
        tags: item.tags || [],
        snippet: item.content
          ? item.content.replace(/<[^>]*>/g, '').substring(0, SNIPPET_MAX_LENGTH)
          : '',
        posted,
      });
    }
    ...
    for (const item of data.jobs) {
      ...
      const posted =
        item.publishedAt || item.publishedDate || item.createdAt
          ? String(item.publishedAt || item.publishedDate || item.createdAt).split('T')[0]
          : 'Unknown';
      if (!isFreshEnough(posted)) continue;
...
async function scanOnce(q, loc) {
  ...
  if (searchConfig.portals.greenhouse) tasks.push(scanAllBoards(greenhouseBoards, scanGreenhouse));
  if (searchConfig.portals.lever) tasks.push(scanAllBoards(leverBoards, scanLever));
  if (searchConfig.portals.ashby) tasks.push(scanAllBoards(ashbyBoards, scanAshby));
