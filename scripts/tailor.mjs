...
function stripMetaCommentary(text) {
  ...
  if (stripped > 0)
    console.warn(`  Stripped ${stripped} meta-commentary / note paragraph(s) from output`);
  return kept.join('\n').trim() + '\n';
}
...
function verifyATS(cvText, candidate, jobDescription) {
  ...
  if (!hasHeader)
    issues.push('No standard section header found (Experience, Skills, Education, etc.)');
  const tableRows = (cvText.match(/^\|.+\|$/gm) || []).length;
  if (tableRows > 5)
    issues.push('Markdown tables detected — some ATS parsers struggle with tables');
  if (cvText.split('\n').some((line) => line.includes('  ')))
    issues.push('Double-spaced lines detected — may indicate multi-column layout');
  ...
  if (jobDescription && jobDescription.length > 50) {
    ...
    const coverage =
      jdKeywords.length > 0 ? Math.round((covered.length / jdKeywords.length) * 100) : 100;
    console.log(
