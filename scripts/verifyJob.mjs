...
async function checkLinkedIn(company, role) {
  ...
  return {
    found: hasJobs,
    signal: hasJobs ? 'Role found on LinkedIn' : 'Role NOT found on LinkedIn',
  };
}
...
async function checkWellfound(company) {
  ...
  return {
    found: hasHiring,
    signal: hasHiring ? 'Company actively hiring on Wellfound' : 'No active hiring on Wellfound',
  };
}
...
async function main() {
  ...
  if (!company) {
    console.error('Usage: node scripts/verifyJob.mjs "https://..." OR --company "Company" --role "Role"');
    process.exit(1);
  }
  ...
  const [linkedin, wellfound, companyPage, greenhouse, lever, ashby] = await Promise.all([
  ...
  if (trustScore >= 80) {
    ...
    console.log('  MEDIUM TRUST — Some signals missing. Worth applying but verify details.');
  } else if (trustScore >= 40) {
    console.log('  LOW TRUST — Few signals. Apply but don\'t spend too much time on customization.');
  } else {
    console.log(' VERY LOW TRUST — May be a ghost job or expired. Consider skipping.');
  }
