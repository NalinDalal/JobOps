...
function showToday(data) {
    ...
    if ((day.dm || 0) < DAILY_GOALS.dm)
        remaining.push(`${DAILY_GOALS.dm - day.dm} more DM`);
    ...
    if ((day.learn || 0) < DAILY_GOALS.learn)
        remaining.push(
            `${DAILY_GOALS.learn - day.learn} more learning session`,
        );
...
function showWeek(data) {
    ...
    if (totals.apply > 0) {
        ...
        console.log(
            `   ${totals.apply} applications → ~${Math.round(totals.apply * responseRate)} responses → ~${Math.round(totals.apply * responseRate * 0.3)} interviews`,
        );
    }
}
