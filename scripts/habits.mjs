...
function getDay(data, date) {
  if (!data.days[date]) {
    data.days[date] = { apply: 0, dm: 0, outreach: 0, learn: 0, linkedin: 0, project: 0, notes: [] };
  }
...
function showToday(data) {
  ...
  for (const [key, label] of Object.entries(HABIT_LABELS)) {
    ...
    const goalStr = key === 'learn' ? `${done}/1` : key === 'apply' ? `${done}/10` : `${done}/${Math.ceil(goal)}`;
    console.log(`${bar} ${label.padEnd(26)} ${String(done).padStart(4)}   ${goalStr.padEnd(6)}  ${pct}%`);
  }
  ...
  // Show what's left
  const remaining = [];
  if ((day.apply || 0) < DAILY_GOALS.apply) remaining.push(`${DAILY_GOALS.apply - day.apply} more applications`);
  if ((day.dm || 0) < DAILY_GOALS.dm) remaining.push(`${DAILY_GOALS.dm - day.dm} more DM`);
  if ((day.outreach || 0) < DAILY_GOALS.outreach) remaining.push(`${DAILY_GOALS.outreach - day.outreach} more outreach`);
  if ((day.learn || 0) < DAILY_GOALS.learn) remaining.push(`${DAILY_GOALS.learn - day.learn} more learning session`);
...
function showWeek(data) {
  ...
  const totals = { apply: 0, dm: 0, outreach: 0, learn: 0, linkedin: 0, project: 0 };
  ...
  for (const [key, label] of Object.entries(HABIT_LABELS)) {
    ...
    const pct = goal > 0 ? Math.min(100, Math.round((done / goal) * 100)) : (done > 0 ? 100 : 0);
    const bar = pct >= 100 ? '' : pct >= 50 ? '' : '';
    console.log(`${bar} ${label.padEnd(26)} ${String(done).padStart(4)}   ${String(Math.ceil(goal)).padEnd(6)}  ${pct}%`);
  }
  ...
  // Funnel
  if (totals.apply > 0) {
    const responseRate = 0.15; // Estimate
    console.log(`\n Estimated funnel (at 15% response rate):`);
    console.log(`   ${totals.apply} applications → ~${Math.round(totals.apply * responseRate)} responses → ~${Math.round(totals.apply * responseRate * 0.3)} interviews`);
  }
}
