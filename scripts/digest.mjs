...
function categorizeJobs(jobs) {
    return {
        strongMatches: jobs.filter((j) => j.evaluation?.overall >= 4.0).length,
        worthReviewing: jobs.filter(
            (j) => j.evaluation?.overall >= 3.5 && j.evaluation?.overall < 4.0,
        ).length,
        withFlags: jobs.filter((j) => j.evaluation?.redFlags?.length > 0)
            .length,
...
function buildHTML(jobs, fullListNote) {
    ...
    function badge(score) {
        const bgc =
            score >= 4 ? "#2d6a4f" : score >= 3.5 ? "#b45309" : "#6e6e73";
    ...
    const { strongMatches, worthReviewing, withFlags, unscored } =
        categorizeJobs(jobs);
    ...
    const tldrText =
        tldrPills.length > 0
            ...
            : '<span style="color:' +
              textSecondary +
              ';">No strong matches today — keep applying</span>';
    ...
    const strongMatchGroups = companyGroups.filter((g) =>
        g.jobs.some((j) => j.evaluation?.overall >= 4.0),
    );
    const worthReviewingGroups = companyGroups.filter(
        (g) =>
            !strongMatchGroups.includes(g) &&
            g.jobs.some((j) => j.evaluation?.overall >= 3.5),
    );
    const withFlagsGroups = companyGroups.filter((g) =>
        g.jobs.some((j) => j.evaluation?.redFlags?.length > 0),
    );
    const unscoredGroups = companyGroups.filter(
        (g) => !g.jobs.some((j) => j.evaluation?.overall),
    );
    ...
    function renderGroups(groups) {
        return groups.map((group, idx) => {
            ...
            const score = primary.evaluation?.overall
                ? badge(primary.evaluation.overall)
                : "";
...
function buildText(jobs, dateStr, freshCount) {
    ...
    const lines = [
        `JobOps Daily Digest — ${dateStr}`,
        `${freshCount} new job(s)`,
        "",
    ];
    ...
    for (const j of jobs) {
        const score = j.evaluation?.overall
            ? ` [${j.evaluation.overall.toFixed(1)}/5]`
            : "";
        lines.push(`${j.title}${score} — ${j.company} (${j.location})`);
        lines.push(`  ${j.url}`);
        if (j.evaluation?.recommendation)
            lines.push(`  Rec: ${j.evaluation.recommendation}`);
        if (j.evaluation?.redFlags?.length)
            lines.push(`   ${j.evaluation.redFlags.join(" • ")}`);
