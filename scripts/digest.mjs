...
function loadSearchConfig() {
  try {
    ...
    return { score_threshold: SCORE_REVIEW, max_per_digest: 10, max_age_days: 30 };
  }
}
...
function saveSeen(seen) {
  ...
  writeFileSync(
    SEEN_PATH,
    JSON.stringify({ seen: [...seen].sort(), updated: new Date().toISOString() }, null, 2),
  );
}
...
function evaluateJob(job) {
  return new Promise((resolvePromise) => {
    ...
    const child = spawn(process.execPath, [resolve(ROOT, 'scripts/evaluate.mjs'), payload], {
      cwd: ROOT,
    });
    let stdout = '';
    child.stdout.on('data', (d) => (stdout += d));
...
function categorizeJobs(jobs) {
  return {
    strongMatches: jobs.filter((j) => j.evaluation?.overall >= SCORE_STRONG).length,
    worthReviewing: jobs.filter(
      (j) => j.evaluation?.overall >= SCORE_REVIEW && j.evaluation?.overall < SCORE_STRONG,
    ).length,
    withFlags: jobs.filter((j) => j.evaluation?.redFlags?.length > 0).length,
    unscored: jobs.filter((j) => !j.evaluation?.overall).length,
  };
}

function buildHTML(jobs, fullListNote) {
  const {
    accent,
    accentLight,
    textPrimary,
  ...
  function badge(score) {
    const bgc = score >= SCORE_STRONG ? '#2d6a4f' : score >= SCORE_REVIEW ? '#b45309' : '#6e6e73';
  ...
  const { strongMatches, worthReviewing, withFlags, unscored } = categorizeJobs(jobs);

  const tldrPills = [];
  if (strongMatches > 0)
    tldrPills.push(
      `<a href="#strong-matches" style="color:#fff;text-decoration:none;">${pill(`${strongMatches} strong match${strongMatches > 1 ? 'es' : ''}`, success)}</a>`,
    );
  if (worthReviewing > 0)
    tldrPills.push(
      `<a href="#worth-reviewing" style="color:#fff;text-decoration:none;">${pill(`${worthReviewing} worth reviewing`, warn)}</a>`,
    );
  if (unscored > 0)
    tldrPills.push(
      `<a href="#unscored" style="color:#fff;text-decoration:none;">${pill(`${unscored} unscored`, textSecondary)}</a>`,
    );
  if (withFlags > 0)
    tldrPills.push(
      `<a href="#with-flags" style="color:#fff;text-decoration:none;">${pill(`${withFlags} with red flags`, danger)}</a>`,
    );
  const tldrText =
    tldrPills.length > 0
      ? tldrPills.join(' ')
      : '<span style="color:' + textSecondary + ';">No strong matches today — keep applying</span>';
  ...
  const strongMatchGroups = companyGroups.filter((g) =>
    g.jobs.some((j) => j.evaluation?.overall >= SCORE_STRONG),
  );
  const worthReviewingGroups = companyGroups.filter(
    (g) =>
      !strongMatchGroups.includes(g) && g.jobs.some((j) => j.evaluation?.overall >= SCORE_REVIEW),
  );
  const withFlagsGroups = companyGroups.filter((g) =>
    g.jobs.some((j) => j.evaluation?.redFlags?.length > 0),
  );
  ...
  function renderGroups(groups) {
    return groups.map((group, idx) => {
      ...
      const rec = primary.evaluation?.recommendation
        ? `<p style="margin:10px 0 0;font-family:${font};font-style:italic;color:${textSecondary};font-size:13px;">${esc(primary.evaluation.recommendation)}</p>`
        : '';
      ...
      const compHtml = compRange
        ? `<span style="margin:0 8px 0 0;font-family:${font};font-size:12px;font-weight:600;color:${success};">${esc(compRange)}</span>`
        : '';

      // Vary outreach by role when there are multiple roles for the same company
      let outreachHtml = '';
      if (group.jobs.length === 1) {
        outreachHtml = esc(outreachFor(primary));
      } else {
        // Generate slightly varied outreach for each role to avoid copy-paste look
        const roleOutreaches = group.jobs
          .slice(0, 3)
          .map((j, i) => {
          ...
          .join(
            '<table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%" style="margin:10px 0;"><tr><td style="border-top:1px dashed ${border};font-size:0;line-height:0;">&nbsp;</td></tr></table>',
          );
        outreachHtml = roleOutreaches;
      }

      const linkedinTitles = (
        getProfileOutreach(loadActiveProfile()).linkedin_titles || [
          'Engineering Manager',
          'Tech Lead',
          'CTO',
          'HR',
      ...
      const peopleCells = linkedinTitles
        .map((t) => {
  ...
  const rows = [
    strongMatchGroups.length > 0
      ? sectionHeader('strong-matches', `${strongMatches} strong matches`, success)
      : '',
    ...renderGroups(strongMatchGroups),
    worthReviewingGroups.length > 0
      ? sectionHeader('worth-reviewing', `${worthReviewing} worth reviewing`, warn)
      : '',
    ...renderGroups(worthReviewingGroups),
    unscoredGroups.length > 0 && withFlagsGroups.length === 0
      ? sectionHeader('unscored', `${unscored} unscored`, textSecondary)
      : '',
    ...renderGroups(unscoredGroups),
    withFlagsGroups.length > 0
      ? sectionHeader('with-flags', `${withFlags} with red flags`, danger)
      : '',
...
async function main() {
  ...
  for (const j of [...scored, ...unscored]) {
    if (isSeniorRole(j.title, j.snippet) && j.evaluation?.overall >= SCORE_STRONG) {
      j.evaluation = j.evaluation || {};
      j.evaluation.overall = Math.min(j.evaluation.overall, SCORE_REVIEW);
      j.evaluation.recommendation =
        j.evaluation.recommendation || 'Seniority mismatch — review before applying';
  ...
  if (!isCI && fresh.length > digestJobs.length) {
    const fullHtml = buildHTML(fresh, '');
    const fullReportFile = resolve(
      reportsDir,
      `digest-full-${ist.toISOString().split('T')[0]}.html`,
    );
    writeFileSync(fullReportFile, fullHtml);
    fullListNote = `<p style="margin:0 0 14px;font-family:${STYLE.font};font-size:12px;color:${STYLE.textSecondary};line-height:1.4;">Full list: <code style="background:${STYLE.accentLight};padding:2px 6px;border-radius:4px;font-size:11px;">${fullReportFile}</code></p>`;
  }

  const text = buildText(digestJobs, dateStr, fresh.length);
  const html = buildHTML(digestJobs, fullListNote);
  ...
  console.log(
    sent ? 'Done.' : 'Preview only — configure RESEND_API_KEY or SMTP credentials to email.',
  );
}
