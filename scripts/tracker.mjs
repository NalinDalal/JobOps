...
import { FOLLOWUP_DEFAULT_DAYS, SEVEN_DAYS_MS } from "./lib/constants.mjs";
...
function listAttention() {
    ...
    for (const row of attention) {
        console.log(
            `| ${row.num} | ${row.company} | ${row.role} | ${row.score} | ${row.lastUpdate} |`,
        );
    }
}
...
function outcomeReview() {
    ...
    if (withOutcome.length === 0) {
        console.log(
            'No outcomes recorded yet. Record outcomes first with: node scripts/tracker.mjs outcome "Company" "Result"',
        );
        return;
    }
    const counts = {};
    ...
    const offers = withOutcome.filter(
        (r) => r.outcome === "Offer Accepted" || r.outcome === "Offer Received",
    );
    const rejected = withOutcome.filter(
        (r) => r.outcome === "Rejected" || r.outcome === "Ghosted",
    );
    ...
    if (offers.length >= 2) {
        console.log(
            "- You have multiple offers. Consider negotiating or declining politely.",
        );
    }
    if (rejected.length >= 3) {
        console.log(
            "- Multiple rejections detected. Consider reviewing your CV or lowering target seniority.",
        );
    }
    const attentionCount = rows.filter((r) => r.status === "Attention").length;
    if (attentionCount > 0) {
        console.log(
            `- ${attentionCount} application(s) in attention queue need review before applying.`,
        );
    }
}
...
function autonomyStatus() {
    ...
    console.log(
        `Source: ${profile.source}${profile.slug ? ` (${profile.slug})` : ""}`,
    );
    if (level === "review-each") {
        console.log(
            "Behavior: All new applications go to Attention queue. You must approve each one before it moves to Saved/Applied.",
        );
    } else if (level === "routine-auto") {
        console.log(
            "Behavior: Applications bypass Attention queue and go directly to Saved. Use with caution.",
        );
    } else {
...
function list() {
    ...
    console.log("\n## Application Tracker\n");
    console.log(
        "| # | Company | Role | Status | Score | Interview | Outcome |",
    );
    console.log(
        "|---|---------|------|--------|-------|-----------|---------|",
    );
    ...
    if (upcomingFollowups.length > 0) {
        ...
        for (const row of upcomingFollowups) {
            console.log(
                `- [${row.followupDate}] ${row.company}: ${row.followupNote}`,
            );
        }
    }
}
...
function add(company, role) {
    ...
    if (dup) {
        console.log(
            `Duplicate detected: ${company} — ${role} already exists (#${dup.num}, status: ${dup.status}). Skipping add.`,
        );
        return;
    }
    ...
    if (initialStatus === "Attention") {
        console.log(
            `Added: ${company} — ${role} (Status: Attention — review required before applying)`,
        );
    } else {
...
function addFollowup(company, note, date) {
    ...
    const followupDate =
        date ||
        new Date(Date.now() + SEVEN_DAYS_MS).toISOString().split("T")[0]; // Default +7 days
    let found = false;
...
function exportCSV() {
    const rows = readTracker();
    let csv =
        "Number,Company,Role,Status,Applied,Score,LastUpdate,InterviewStage,Outcome,FollowupDate,FollowupNote\n";
    for (const row of rows) {
...
function generateReport() {
    const child = spawn(
        process.execPath,
        [resolve(__dirname, "htmlReport.mjs")],
        {
            stdio: "inherit",
            cwd: ROOT,
        },
...
switch (command) {
    ...
    default:
        ...
        console.log(
            "  node scripts/tracker.mjs attention              — Show attention queue",
        );
        console.log(
            "  node scripts/tracker.mjs review                — Outcome review + suggestions",
        );
        console.log(
            "  node scripts/tracker.mjs autonomy              — Show autonomy level",
        );
        console.log(
            "  node scripts/tracker.mjs reset <mode>          — Reset tracker (profile|documents|all)",
        );
}
...
function handleReset() {
    ...
    console.log(
        "   - documents: deletes archived application folders under data/applications/",
    );
