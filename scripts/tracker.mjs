...
import { readTracker, writeTracker, findRow, todayKey } from './lib/tracker.mjs';
...
function addFollowup(company, note, date) {
  const rows = readTracker();
  const followupDate = date || new Date(Date.now() + SEVEN_DAYS_MS).toISOString().split('T')[0]; // Default +7 days
  let found = false;
...
function generateReport() {
  const child = spawn(process.execPath, [resolve(__dirname, 'htmlReport.mjs')], {
    stdio: 'inherit',
    cwd: ROOT,
  });
  child.on('error', (err) => {
