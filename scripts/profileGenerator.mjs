                ...
                messages: [
                    {
                        role: "system",
                        content:
                            "You are a resume parser. Extract structured data and return ONLY valid YAML.",
                    },
...
function extractSkillsFromResume(sections) {
    const skillsText =
        sections["full_skills_inventory_granular_trim_per_jd"] || "";
    const skills = {
        languages: [],
        frameworks: [],
        databases: [],
...
function extractExperience(sections) {
    const expText =
        sections["experience_full_detail_more_bullets_than_the_trimmed_tex"] ||
        "";
    const experience = [];
...
async function fetchGitHubData() {
    try {
        ...
        const reposRes = await fetch(
            "https://api.github.com/users/NalinDalal/repos?per_page=100&sort=updated",
            {
...
async function fetchCPData() {
    ...
    try {
        ...
        if (lcData.data?.matchedUser) {
            ...
            cpData.leetcode = {
                total,
                ranking: lcData.data.matchedUser.profile?.ranking,
            };
        }
    } catch (e) {
    ...
    try {
        ...
        const cfRes = await fetch(
            "https://codeforces.com/api/user.info?handles=nalindalal2004",
        );
...
async function main() {
    ...
    const resumePath = customFile
        ? resolve(ROOT, customFile)
        : resolve(ROOT, "config/resume.md");
    ...
    console.log(
        `   Skills categories: ${Object.keys(skills)
    ...
    if (enrich) {
        ...
        if (githubData) {
            console.log(
                `   Public repos: ${githubData.publicRepos}, Stars: ${githubData.totalStars}, Contributions: ${githubData.totalContributions}`,
            );
        }
        ...
        if (cpData.leetcode)
            console.log(`   LeetCode: ${cpData.leetcode.total} solved`);
        if (cpData.codeforces)
            console.log(
                `   Codeforces: ${cpData.codeforces.rating} (${cpData.codeforces.rank})`,
            );
    }
...
location_preferences:
  ...
  negative: []`;
    ...
    console.log(
        "   Key fields to verify: target_roles, skills, experience.level, target_locations",
    );
}
