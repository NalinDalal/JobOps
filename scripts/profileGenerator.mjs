    ...
    `https://api.cloudflare.com/client/v4/accounts/${CF_ACCOUNT}/ai/run/${CF_MODEL}`,
    {
      ...
      body: JSON.stringify({
        messages: [
          {
            role: 'system',
            content: 'You are a resume parser. Extract structured data and return ONLY valid YAML.',
          },
...
async function fetchGitHubData() {
  try {
    ...
    return {
      ...
      totalContributions:
        graphqlData.data?.user?.contributionsCollection?.contributionCalendar?.totalContributions ||
        0,
    };
  } catch (e) {
...
async function main() {
  ...
  if (enrich) {
    ...
    if (cpData.codeforces)
      console.log(`   Codeforces: ${cpData.codeforces.rating} (${cpData.codeforces.rank})`);
  }
