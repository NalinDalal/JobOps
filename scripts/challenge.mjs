...
import {
    CHALLENGE_DAYS,
    CHALLENGE_TOTAL_GOAL,
    CHALLENGE_DAILY_GOAL,
    RESPONSE_RATE,
    INTERVIEW_RATE,
    OFFER_RATE,
...
function loadChallenge() {
    try {
        ...
        console.warn(`Warning: Could not load challenge data: ${e.message}`);
    }
    return {
        startDate: new Date().toISOString().split("T")[0],
        goal: CHALLENGE_TOTAL_GOAL,
        dailyGoal: CHALLENGE_DAILY_GOAL,
        days: {},
    };
}
...
function showToday(data) {
    ...
    const totalOutreach = Object.values(data.days).reduce(
        (sum, d) => sum + (d.companies?.length || 0),
        0,
    );
    const daysActive = Object.keys(data.days).length;

    console.log(`\n Day ${dayNum}/${CHALLENGE_DAYS} — ${today}\n`);

    console.log(
        `   Today's progress: ${day.companies?.length || 0}/${data.dailyGoal} companies`,
    );

    if (day.companies?.length > 0) {
        console.log("\n   Companies reached out to today:");
        day.companies.forEach((c) =>
            console.log(`      ${c.company} — ${c.method}`),
        );
    }
    ...
    console.log("\n   ───────────────────────────────────────");
    console.log(
        `    Overall: ${totalOutreach}/${data.goal} companies (${Math.round((totalOutreach / data.goal) * 100)}%)`,
    );
    console.log(`    Days active: ${daysActive}/${CHALLENGE_DAYS}`);
    console.log(
        `    Avg/day: ${(totalOutreach / Math.max(1, daysActive)).toFixed(1)}`,
    );

    // Estimate completion
    const remaining = data.goal - totalOutreach;
    const daysLeft = CHALLENGE_DAYS - dayNum;
    ...
    if (daysLeft > 0) {
        ...
        if (neededPerDay > data.dailyGoal) {
            console.log(
                `     You're behind pace! Consider increasing daily outreach.`,
            );
        } else {
...
function showStats(data) {
    ...
    const totalEmails = Object.values(data.days).reduce(
        (sum, d) => sum + (d.emails || 0),
        0,
    );
    const totalLinkedin = Object.values(data.days).reduce(
        (sum, d) => sum + (d.linkedin || 0),
        0,
    );
    const totalCalls = Object.values(data.days).reduce(
        (sum, d) => sum + (d.calls || 0),
        0,
    );
    const daysActive = Object.keys(data.days).length;
    ...
    console.log(`\n Challenge Stats (Day ${dayNum}/${CHALLENGE_DAYS})\n`);
    ...
    console.log(
        `   Days active                ${daysActive}/${CHALLENGE_DAYS}`,
    );
    console.log(
        `   Avg per day                ${(totalOutreach / Math.max(1, daysActive)).toFixed(1)}`,
    );

    // Funnel estimate
    const responses = Math.round(totalOutreach * RESPONSE_RATE);
    const interviews = Math.round(responses * INTERVIEW_RATE);
    const offers = Math.round(interviews * OFFER_RATE);

    console.log("\n    Estimated funnel (15% response rate):");
    console.log(
        `      ${totalOutreach} outreach → ${responses} responses → ${interviews} interviews → ${offers} offers`,
    );
...
switch (command) {
    ...
    case "log":
        if (!args[1]) {
            console.error(
                'Usage: node scripts/challenge.mjs log "Company" [method]',
            );
