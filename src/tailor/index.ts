...
import { loadActiveProfile, getProfileSkills, getProfileExperience } from "../lib/profile.js";
import { CV_TRUNCATE } from "../lib/constants.js";
...
export async function runTailor(options: TailorOptions): Promise<TailorResult> {
  ...
  if (!existsSync(CV_PATH)) {
    throw new Error(`CV not found at ${CV_PATH}. Please create config/cv.md`);
  }
  ...
  const cvOutputPath = resolve(OUTPUT_DIR, `${safeCompany}-${safeRole}-cv-${timestamp}.md`);
  const clOutputPath = resolve(OUTPUT_DIR, `${safeCompany}-${safeRole}-cover-letter-${timestamp}.md`);
