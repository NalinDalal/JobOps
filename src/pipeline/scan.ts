...
import type { Job, JobSource } from "../domain/job";
import { createJobId } from "../domain/job";
import { validateJobSafe } from "../schemas/job";
import { loadSearchConfig, loadPortalsConfig } from "../config/loader";
import { loadActiveProfile, getProfileTargetRoles } from "../lib/profile";
import { FETCH_TIMEOUT_MS, SNIPPET_MAX_LENGTH } from "../lib/constants";
...
export async function scanJobs(options: ScanOptions = {}): Promise<ScanResult> {
    ...
    for (let i = 0; i < results.length; i++) {
        const r = results[i];
        const sourceName = ["remoteok", "arbeitnow", "findwork", "remotive"][
            i
        ]!;
