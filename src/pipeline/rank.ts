...
function downgradeSeniorRoles(jobs: Job[]): void {
  for (const job of jobs) {
    if (
      ...
      job.evaluation.overall = Math.min(job.evaluation.overall, SCORE_REVIEW);
      job.evaluation.recommendation =
        job.evaluation.recommendation ||
        "Seniority mismatch — review before applying";
      if (!job.evaluation.redFlags.includes("JD appears senior-level; confirm junior/entry fit")) {
        job.evaluation.redFlags.push("JD appears senior-level; confirm junior/entry fit");
      }
    }
  }
}
