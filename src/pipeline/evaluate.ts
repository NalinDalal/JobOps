...
import type { JobEvaluation } from "../domain/evaluation.js";
import { scoreToVerdict } from "../domain/evaluation.js";
import { loadActiveProfile, getProfileSkills, getProfileExperience, getProfileTargetRoles, getProfileTargetLocations } from "../lib/profile.js";
import { SCORE_STRONG, SCORE_REVIEW, CV_TRUNCATE } from "../lib/constants.js";
import { loadEnv, hasCloudflareKeys, type Env } from "../config/env.js";
...
function loadProfileForEvaluation() {
  ...
  return {
    ...
    salary: (profile.data.preferences as Record<string, unknown>)?.salary_range as string || "Negotiable",
  };
}
...
Return JSON: {"overall":4.2,"roleFit":4.5,"locationFit":4.0,"growthPotential":4.5,"compFit":4.0,"cultureFit":4.0,"entryLevelFit":4.0,"recommendation":"Apply!","analysis":"...","redFlags":[]}`;
  ...
  try {
    ...
    console.error(`Evaluation failed for ${job.title} at ${job.company}: ${e}`);
    return null;
  }
}
...
export async function evaluateJobs(
  ...
  console.log(`Evaluating top ${Math.min(jobs.length, concurrent)} jobs with Cloudflare AI...`);
