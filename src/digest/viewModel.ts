...
import type { DigestViewModel, DigestMatch, DigestSummary, DigestDate, DigestFooter, MatchScore } from "../domain/digest.js";
import type { OutreachGroup, PeopleSearchUrl } from "../domain/outreach.js";
import type { SkillSignal } from "../domain/skill.js";
import { scoreToVerdict, topLabel } from "../domain/evaluation.js";
import { loadActiveProfile, getProfileSkills, getProfileTargetRoles, getProfileTargetLocations, getProfileCandidate, getProfileExperience, getProfileOutreach } from "../lib/profile.js";
import { normalizeLocation, truncate } from "../lib/text.js";
import { SCORE_STRONG, SCORE_REVIEW, TRUNCATE_DEFAULT } from "../lib/constants.js";
...
const STOP_WORDS = new Set([
  "the", "and", "for", "with", "you", "your", "are", "has", "have",
  "this", "that", "will", "can", "our", "job", "role", "team", "work",
  "working", "looking", "seeking", "candidate", "experience", "years",
  "year", "strong", "excellent", "good", "knowledge", "using", "use",
  "used", "etc", "including", "well", "able", "must", "requirements",
  "required", "preferred", "plus", "bonus", "qualifications", "skills",
  "responsibilities",
]);
...
const WEB_PRIORITY_SKILLS = [
  ...
  "tailwind css", "elysia", "express", "postgresql", "prisma",
  "mongodb", "redis", "docker", "kubernetes", "aws", "github actions",
  "bun", "jest", "bash", "linux",
];
...
const SENIOR_INDICATORS = [
  ...
  "vp", "head of", "architect", "fellow", "distinguished",
  "staff engineer", "principal engineer", "iii", "iv", "v",
...
function buildPeopleSearchUrls(company: string, linkedinTitles: string[]): PeopleSearchUrl[] {
...
const SKILL_GAP_BLACKLIST = new Set([
  "software", "engineer", "engineering", "development", "developer",
  "technology", "technical", "system", "systems", "platform", "platforms",
  "service", "services", "application", "applications", "project", "projects",
  "team", "teams", "company", "companies", "product", "products",
  "data", "information", "process", "solutions", "solution",
  "build", "building", "built", "working", "experience",
  "knowledge", "understanding", "ability", "skills", "familiar",
  "proficiency", "proficient", "expertise", "expert",
  "requirement", "requirements", "qualification", "qualifications",
  "responsibility", "responsibilities", "duties", "role",
  "environment", "culture", "collaboration", "communication",
  "problem", "solving", "analysis", "analytical",
  "design", "architecture", "infrastructure",
  "management", "leadership", "mentoring",
  "agile", "scrum", "methodology", "best", "practices",
  "testing", "quality", "debugging", "troubleshooting",
  "documentation", "maintenance", "performance", "scalability",
  "reliability", "integration", "deployment", "automation",
  "innovation", "optimization", "improvement",
  "initiative", "proactive", "self-motivated",
  "detail", "oriented", "organized", "efficient",
  "independent", "collaborative", "cross-functional",
]);

function analyzeSkillGaps(jobs: Job[], allSkills: string[]): SkillSignal | null {
  ...
  for (const [skill, count] of skillCounts) {
    ...
    if (frequency >= 0.25 && !lowerSkills.has(skill) && !SKILL_GAP_BLACKLIST.has(skill)) {
...
function buildMatch(
  ...
  return {
    ...
    score,
    verdict: evaluation?.overall ? scoreToVerdict(evaluation.overall) : "Unscored",
...
export function buildViewModel(jobs: Job[], options: ViewModelOptions = {}): DigestViewModel {
  ...
  // Date formatting
  const now = new Date();
  const dateStr = options.dateStr || now.toISOString().replace("T", " ").substring(0, 16);
  ...
  // Summary stats
  const strongMatches = jobs.filter((j) => j.evaluation?.overall && j.evaluation.overall >= SCORE_STRONG).length;
  const worthReviewing = jobs.filter(
    (j) => j.evaluation?.overall && j.evaluation.overall >= SCORE_REVIEW && j.evaluation.overall < SCORE_STRONG
  ).length;
  const companies = new Set(jobs.map((j) => j.company));

  // Build all matches
  const topMatches = jobs.map((job, idx) => buildMatch(job, idx, allSkills, targetLocations));
