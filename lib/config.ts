...
export function loadEnv(root?: string): Env {
    ...
    cachedEnv = {
        ...
        cloudflareModel:
            process.env.CLOUDFLARE_MODEL || DEFAULT_ENV.cloudflareModel,
...
export function loadPortalsConfig(): PortalsConfigInput {
    ...
    const pickBoards = (key: string): import("./types").PortalEntry[] => {
        const v = raw[key] as
            | { boards?: import("./types").PortalEntry[] }
            | import("./types").PortalEntry[]
            | undefined;
        if (!v) return [];
    ...
    return {
        ...
        blacklist:
            (raw.blacklist as PortalsConfigInput["blacklist"]) || undefined,
        whitelist:
            (raw.whitelist as PortalsConfigInput["whitelist"]) || undefined,
        title_filter:
            (raw.title_filter as PortalsConfigInput["title_filter"]) ||
            undefined,
        search_queries:
            (raw.search_queries as PortalsConfigInput["search_queries"]) ||
            undefined,
    };
}
...
function loadYamlProfile(path: string): Profile {
    try {
        ...
        return (
            (yamlLoad(readFileSync(path, "utf-8")) as Profile) ||
            ({} as Profile)
        );
    } catch {
        return {} as Profile;
    ...
    if (slug) {
        ...
        if (existsSync(presetPath)) {
            return {
                source: "preset",
                slug,
                data: loadYamlProfile(presetPath),
            };
        }
    }
