...
export async function generateOutreach(
    ...
    if (!hasCloudflareKeys(env)) {
        console.warn(
            "No Cloudflare AI keys found, using template-based outreach",
        );
...
function saveOutreachDraft(draft: OutreachDraft): string {
    ...
    const safeCompany = draft.company
        .replace(/[^a-zA-Z0-9]/g, "-")
        .toLowerCase();
...
export async function generateOutreachForCompanies(
    ...
    for (const company of targets) {
        try {
            ...
            console.error(
                `Failed to generate outreach for ${company.name}: ${e}`,
            );
        }
    }
...
if (import.meta.main) {
    ...
    const role =
        args.find((a) => a.startsWith("--role="))?.slice(6) || "Engineer";
    const format = (args.find((a) => a.startsWith("--format="))?.slice(8) ||
        "short-dm") as "short-dm" | "long-dm" | "email";

    if (!company) {
        console.error(
            "Usage: outreach <company> [--role <role>] [--format <short-dm|long-dm|email>]",
        );
        process.exit(1);
    }
