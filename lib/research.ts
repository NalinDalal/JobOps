...
import type { AcceleratorCompany, AcceleratorResult, ResearchResult, RawAccelerator, RawResearchConfig } from "./types";
...
function parseAcceleratorPage(
  ...
  for (const block of blocks) {
    ...
    const techStack = guessTechStack(text + " " + (accelerator.tags || []).join(" "));

    seen.add(name.toLowerCase());
    companies.push({
      name,
      url: url || `${accelerator.company_list_url}/${name.toLowerCase().replace(/\s+/g, "-")}`,
      careersUrl: accelerator.careers_pattern.replace("{slug}", name.toLowerCase().replace(/\s+/g, "-")),
...
export async function researchAccelerators(
  ...
  const accelerators = config.accelerators.filter((a) => {
    if (options.acceleratorSlug && a.slug !== options.acceleratorSlug) return false;
  ...
  for (const accelerator of accelerators) {
    const accResult: AcceleratorResult = {
      ...
      batch: options.batch || accelerator.batch_urls[0]?.batch || "unknown",
      companies: [],
      error: undefined,
    };
    ...
    try {
      const batchesToFetch = options.batch
        ? accelerator.batch_urls.filter((b) => b.batch === options.batch)
      ...
      for (const batch of batchesToFetch) {
        try {
          console.log(`  Fetching ${accelerator.name} (${batch.label})...`);
          const html = await fetchWithTimeout(batch.url);
          const companies = parseAcceleratorPage(html, accelerator, maxPerAccelerator);

          for (const company of companies) {
            if (!accResult.companies.find((c: AcceleratorCompany) => c.name === company.name)) {
...
if (import.meta.main) {
  ...
  const batch = args.find((a) => a.startsWith("--batch="))?.slice(7) || undefined;
