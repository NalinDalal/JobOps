    ...
    try {
        ...
        for (const item of data.results || []) {
            const job: Job = {
                ...
                snippet: String(item.text || "").substring(
                    0,
                    SNIPPET_MAX_LENGTH,
                ),
        ...
        try {
            ...
            for (const item of data.jobs || []) {
                const job: Job = {
                    ...
                    location: normalizeLocation(
                        String(
                            (item.location as Record<string, unknown>)?.name ||
                                location,
                        ),
                    ),
                    url: String(item.absolute_url || ""),
                    description: String(item.content || ""),
                    snippet: String(item.content || "").substring(
                        0,
                        SNIPPET_MAX_LENGTH,
                    ),
...
function matchesTitleFilters(
    ...
    portalsConfig: ReturnType<typeof loadPortalsConfig>,
): boolean {
...
function matchesCompanyFilters(
    ...
    if (bl?.enabled && bl.companies.length > 0) {
        if (bl.companies.some((c) => company.includes(c.toLowerCase())))
            return false;
    }
    return true;
}
...
function matchesQuery(job: Job, query: string): boolean {
    if (!query || query === "auto" || query === "software engineer")
        return true;
    const tokens = query.toLowerCase().split(/\s+/).filter(Boolean);
    if (tokens.length === 0) return true;
    const hay =
        `${job.title} ${job.description || ""} ${job.snippet || ""} ${(job.tags || []).join(" ")}`.toLowerCase();
...
export function filterJobs(jobs: Job[], query: string): Job[] {
    ...
    return jobs.filter(
        (j) =>
            matchesTitleFilters(j, searchConfig, portalsConfig) &&
            matchesCompanyFilters(j, portalsConfig) &&
            matchesAgeFilter(j, searchConfig.max_age_days) &&
...
export async function scanJobs(options: ScanOptions = {}): Promise<ScanResult> {
    ...
    if (preValidate !== result.jobs.length)
        console.log(`Dropped ${preValidate - result.jobs.length} invalid rows`);

    const preFilter = result.jobs.length;
    result.jobs = filterJobs(result.jobs, query);
    const filtered = preFilter - result.jobs.length;
    if (filtered > 0)
        console.log(`Filtered ${filtered} jobs by title/company/age/query`);
